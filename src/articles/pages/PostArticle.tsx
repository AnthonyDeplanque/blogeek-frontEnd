  ...
  return (
    auth?.roles?.includes(ROLE.ROLE_CREATOR) || auth?.roles?.includes(ROLE.ROLE_ADMIN) ? (
      <MainCard title="Poster un article" display="flex" flexDirection="row" alignItems="center" justifyContent="stretch">
        <Box width="100%" padding={theme.spacing(2)}><TextField style={{ width: "100%" }} name="title" onChange={(e) => setTitle(e.target.value)}>
        </TextField></Box>
        <Box width="100%" padding={theme.spacing(2)}><TextField style={{ width: "100%" }} name="subtitle" onChange={(e) => setSubtitle(e.target.value)}>
        </TextField></Box>
        <Box width="100%" padding={theme.spacing(2)}><TextField style={{ width: "100%" }} name="content" onChange={(e) => setContent(e.target.value)}>
        </TextField></Box>
        <Button variant="contained" onClick={handlePostArticle}>Poster l'article</Button>
      </MainCard>)
